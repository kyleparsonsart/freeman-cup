...
| Tests | 69 unit (26 scoring incl. aggregate + 14 write queue + 8 feed + 6 moments + 6 card + 5 standings + 4 moment overlays). 12 RLS integration tests unchanged; run `npm test` from a real terminal (sandboxed agents can't reach Supabase) |

SQL that has run, in order: `freeman-cup-schema.sql`, `freeman-cup-auth.sql`, `freeman-cup-handoff.sql`, `freeman-cup-commish.sql`, `freeman-cup-shootout.sql` (run Sep 3), `freeman-cup-cards.sql` (run Sep 3), `freeman-cup-aggregate.sql` (**Sep 3 — not yet run; paste into the SQL editor once**). Each is the record of what ran; don't re-run the schema.
