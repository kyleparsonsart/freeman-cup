...
| MVP hole points (Sep 8, halves dropped Sep 9) | `standings.ts` `holePoints()`: 3 for a hole your ball won alone, 2 for one your side won together (both partners held the best net, or any aggregate win), nothing for a halve, a loss or a pick-up; on the Scoring tab a won cell wears the stars (two alone, one together), drawn from the derived result so they vanish on a correction; singles wins always 3; uses the derived hole result so bye holes count. MVP board sorts by points, net against par breaks ties (and stays as the Medalist line); Player of the Round is most points in the round among full cards. Rulebook Art. 11, MVP chip in quick search; finale MVP line and the player card's MVP points tile read from the board. Six unit tests walk the worked examples |
...
- **Celts first, Vikes second, everywhere the two sides sit together** (Sep 9): `SIDES` and `lineup()` in scoring.ts name the order; the cup bars grow from their own side (blue from the left), the lead line puts the Celts above the line, group-tab gradients come from the leader's side. Side keys are unchanged (`a` Vikes, `b` Celts); this is display order only. "X lead, m to n" lines keep the leader's number first.
...
- **Hole points are 2 alone, 1 together** (Sep 9, was 3/2). `POINTS` in standings.ts; Art. 11, the Cup page copy and the public site all say so. Player of the round stays in the app only, not on thefreemancup.com.
