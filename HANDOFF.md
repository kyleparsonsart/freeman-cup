...
| MVP hole points (Sep 8) | `standings.ts` `holePoints()`: 3 for a hole your ball won alone, 2 for one your side won together (both partners held the best net, or any aggregate win), 1 to all four for a halve, 0 for a loss or a pick-up; singles wins always 3; uses the derived hole result so bye holes count. MVP board sorts by points, net against par breaks ties (and stays as the Medalist line); Player of the Round is most points in the round among full cards. Rulebook Art. 11, MVP chip in quick search; finale MVP line and the player card's MVP points tile read from the board. Six unit tests walk the worked examples |
