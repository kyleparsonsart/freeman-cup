...
| Tests | 44 unit (21 scoring + 10 write queue + 7 feed + 6 moments). 12 RLS integration tests unchanged; run `npm test` from a real terminal (sandboxed agents can't reach Supabase) |

SQL that has run, in order: `freeman-cup-schema.sql`, `freeman-cup-auth.sql`, `freeman-cup-handoff.sql`, `freeman-cup-commish.sql`, `freeman-cup-shootout.sql` (**Sep 2 — not yet run; paste into the SQL editor once**). Each is the record of what ran; don't re-run the schema.
